...
export default function SamplePrimaryInfoPage() {
  ...
  const loadStyles = async () => {
    try {
      // Load style variants instead of style summaries
      const response = await fetch("http://localhost:8000/api/v1/samples/style-variants");
      if (response.ok) {
        const data = await response.json();
      ...
      console.error("Failed to load style variants:", error);
      setStyles([]);
    }
  };
  ...
  const handleStyleChange = (styleVariantId: string) => {
    // styleVariantId represents style_variant_id
    const styleVariant = styles.find((s) => s.id.toString() === styleVariantId);
    if (styleVariant) {
      setFormData({
        ...formData,
        style_variant_id: styleVariantId, // Store the selected variant id
        style_id: styleVariant.style_summary_id.toString(), // Use the style_summary_id for the sample
        style_name: `${styleVariant.style_name} - ${styleVariant.colour_name}`,
        item: styleVariant.style_category || "",
        gauge: styleVariant.gauge || "",
      });
    }
  };
  ...
  const handleEdit = (sample: any) => {
    ...
    setFormData({
      ...sample,
      style_variant_id: sample.style_variant_id || "", // Ensure style_variant_id exists
    });
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={formData.style_variant_id || undefined}
                  ...
                  >
                    ...
                    <SelectContent>
                      {styles.map((styleVariant) => (
                        <SelectItem key={styleVariant.id} value={styleVariant.id.toString()}>
                          {styleVariant.style_name} - {styleVariant.colour_name} ({styleVariant.style_id})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {styles.length === 0 && (
                    <p className="text-xs text-destructive">No style variants available. Please add a style variant first.</p>
                  )}
                </div>
              </div>
