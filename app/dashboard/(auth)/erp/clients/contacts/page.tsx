...
export default function ContactsPage() {
  ...
  const loadContacts = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/v1/buyers/contacts");
      if (response.ok) {
  ...
  const clearFilters = () => {
    setFilters({ search: "", companyType: "", country: "" });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingContact
        ? `http://localhost:8000/api/v1/buyers/contacts/${editingContact.id}`
        : "http://localhost:8000/api/v1/buyers/contacts/";
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this contact?")) {
      try {
        await fetch(`http://localhost:8000/api/v1/buyers/contacts/${id}`, {
          method: "DELETE",
        });
