...
export default function ShippingPage() {
  ...
  const loadShipping = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/v1/buyers/shipping");
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingShipping
        ? `http://localhost:8000/api/v1/buyers/shipping/${editingShipping.id}`
        : "http://localhost:8000/api/v1/buyers/shipping";
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this shipping info?")) {
      try {
        await fetch(`http://localhost:8000/api/v1/buyers/shipping/${id}`, {
          method: "DELETE",
        });
